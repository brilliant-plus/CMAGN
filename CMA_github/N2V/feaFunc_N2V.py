...
# ：param graph：输入图
# ：param dimensions:嵌入维度（默认值：128）
# ：param walk_length：每个行走中的节点数（默认值：80）
# ：param num_walks:每个节点的行走次数（默认值：10）
# ：param p：返回超参数（默认值：1）
# ：param q：Inout参数（默认值：1）
# ：param weight_key：在加权图上，这是weight属性的键（默认值：'weight'）
# ：param workers：并行执行的工作线程数（默认值：1）
# ：param sampling_strategy：节点特定的采样策略，支持设置节点特定的“q”、“p”、“num_walks”和“walk_length”。
...
def N2V(edge_path, para_dim, random_seed):
    ...
    # 方法二等效方法一
    # G = nx.read_edgelist(edge_path, create_using=nx.DiGraph(), nodetype=None,
    #                      data=[('weight', float)])
